...
export default function Projects() {
  ...
  useEffect(() => {
    const fetchCaseStudies = async (isInitialFetch = false) => {
      if (isInitialFetch) setLoadingCaseStudies(true);
  ...
  useEffect(() => {
    ...
    const animateCarousel = (currentTime: number) => {
      const deltaTime = currentTime - lastTime;
      lastTime = currentTime;
      const frameSpeed = animationSpeed * (deltaTime / 16.67);

      if (!isPausedRef.current) {
        currentPosition -= frameSpeed;
        if (currentPosition <= -singleLoopWidth)
          currentPosition += singleLoopWidth;
        if (carouselRef.current) {
          const track = carouselRef.current.querySelector(
  ...
  const handleCarouselMouseEnter = useCallback(() => {
    isPausedRef.current = true;
    if (carouselRef.current) carouselRef.current.style.opacity = "0.95";
  }, []);
  ...
  const memoizedCaseStudies = useMemo(() => {
    if (caseStudies.length === 0) return [];
    const repeatedArray: CaseStudy[] = [];
    for (let i = 0; i < 3; i++) repeatedArray.push(...caseStudies);
    return repeatedArray;
  }, [caseStudies]);
    ...
    () => (
      ...
      >
        <div className={styles.cardContent}>
          <div className={styles.cardText}>
            <h3>Toingg</h3>
            {hoveredIndex === 3 && <p style={{ opacity: 1 }}>Click to View</p>}
          </div>
          <div className={styles.cardImage}>
            <video
              className={styles.imgTag}
