...
import { fetchAINewsBySlug } from '@/services/ainewsService';
...
const Ainews = () => {
  ...
  useEffect(() => {
    ...
    const fetchAinews = async () => {
      ...
      setError('');
      try {
        const data = (await fetchAINewsBySlug(String(slug))) as unknown as AinewsType;
        setAiNews(data);
        console.log('ainews post data is here', data);
      } catch (err: unknown) {
        setError(err instanceof Error ? err.message : 'Failed to fetch AI news.');
      } finally {
        setLoading(false);
      }
    };
