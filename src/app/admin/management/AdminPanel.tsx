...
import { deleteContent, fetchContentDetails as fetchContentDetailsByType } from '@/services/contentService';
...
const AdminPanel: React.FC = () => {
  ...
  const loadContentDetails = async (slug: string, contentType: ContentType) => {
    setDetailsLoading(true);
    setDetailsError('');
    try {
      setContentDetails(await fetchContentDetailsByType(contentType, slug));
    } catch (error: unknown) {
      console.error('Error fetching content details:', error);
      setDetailsError(error instanceof Error ? error.message : 'Failed to fetch details.');
    } finally {
  ...
  const handleView = (content: ContentSummary) => {
    ...
    setModalOpen(true);
    loadContentDetails(content.slug, content.contentType);
  };

  const handleEdit = async (content: ContentSummary) => {
    try {
      console.log('onLicking edit button', content)
      const data: ContentDetails = await fetchContentDetailsByType(content.contentType, content.slug);
      ...
      alert(error instanceof Error ? error.message : 'Error fetching content details.');
    }
  };
  ...
  const handleDelete = async (content: ContentSummary, fetchContents: ()=> void) => {
    ...
    try {
      await deleteContent(content.contentType, content.slug);
      alert(
